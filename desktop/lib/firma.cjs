/**
 * Firma XAdES-BES con el certificado .p12 de la caja — COPIA EXACTA de
 * src/lib/sri/sign.server.ts del sistema web (node-forge, sin dependencias nativas).
 */
...
function signXmlXades(p12Der, password, xml, signingInstant) {
  let binary = "";
  for (let i = 0; i < p12Der.length; i++) binary += String.fromCharCode(p12Der[i]);
  ...
  const cert =
    certBags
      ...
      .find(
        (c) =>
          c && c.publicKey && privateKey && c.publicKey.n.toString(16) === privateKey.n.toString(16),
      ) ?? (certBags[0] && certBags[0].cert);
  ...
  const modulus = forge.util.encode64(
    forge.util.hexToBytes(
      publicKey.n.toString(16).padStart(Math.ceil(publicKey.n.toString(16).length / 2) * 2, "0"),
    ),
  );
  ...
  const signedPropsC14n = `<etsi:SignedProperties xmlns:ds="${DS}" xmlns:etsi="${ETSI}" Id="${sPropsId}">${signedPropsInner}</etsi:SignedProperties>`;
  ...
  if (!signedXml.startsWith("<?xml")) signedXml = '<?xml version="1.0" encoding="UTF-8"?>\n' + signedXml;
  return { xml: signedXml };
}

module.exports = { signXmlXades };
