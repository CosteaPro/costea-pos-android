...
BEGIN
  ...
  VALUES
    (_company, 'S001', COALESCE(NULLIF(_branch_name,''), 'Matriz'), COALESCE(_branch_address,''),
     COALESCE(NULLIF(_establishment,''), '001'), COALESCE(NULLIF(_emission_point,''), '001'),
     'local', true, true, _actor, _actor)
  ...
  INSERT INTO public.document_sequences
    (company_id, branch_id, doc_type, establishment, emission_point, next_sequential)
  VALUES
    (_company, _branch, 'factura', COALESCE(NULLIF(_establishment,''), '001'),
     COALESCE(NULLIF(_emission_point,''), '001'), 1)
  ON CONFLICT DO NOTHING;

  INSERT INTO public.company_settings
    (company_id, business_name, trade_name, ruc, establishment, emission_point)
  VALUES
    (_company, COALESCE(NULLIF(_legal_name,''), _trade_name), _trade_name, _ruc,
     COALESCE(NULLIF(_establishment,''), '001'), COALESCE(NULLIF(_emission_point,''), '001'))
  ON CONFLICT DO NOTHING;
