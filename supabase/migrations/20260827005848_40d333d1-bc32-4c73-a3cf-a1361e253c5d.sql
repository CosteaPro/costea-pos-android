...
BEGIN
  ...
  -- Costo por unidad: factor inverso al de cantidad (linea -> unidad de receta del item).
  UPDATE public.recipe_items ri
     SET unit_cost = ROUND(COALESCE(i.cost_per_recipe_unit, 0)
                           * public.unit_convert_factor(ri.unit, COALESCE(i.recipe_unit, ri.unit)), 8),
         subtotal = ROUND(COALESCE(ri.quantity, 0) * COALESCE(i.cost_per_recipe_unit, 0)
                           * public.unit_convert_factor(ri.unit, COALESCE(i.recipe_unit, ri.unit)), 6),
         updated_at = now()
    FROM public.inventory_items i
   WHERE i.id = _item_id
     AND ri.item_id = _item_id
     AND ri.source_type = 'item'
     AND COALESCE(public.unit_convert_factor(ri.unit, COALESCE(i.recipe_unit, ri.unit)), 0) <> 0;
  ...
  WHILE v_level < 6 AND COALESCE(array_length(v_touched, 1), 0) > 0 LOOP
    ...
    UPDATE public.recipe_items ri
       SET unit_cost = ROUND(c.costo * public.unit_convert_factor(ri.unit, COALESCE(c.yield_unit, ri.unit)), 8),
           subtotal = ROUND(COALESCE(ri.quantity, 0) * c.costo
                            * public.unit_convert_factor(ri.unit, COALESCE(c.yield_unit, ri.unit)), 6),
           updated_at = now()
      FROM (
     ...
     WHERE ri.sub_recipe_id = c.id
       ...
       AND COALESCE(public.unit_convert_factor(ri.unit, COALESCE(c.yield_unit, ri.unit)), 0) <> 0;

    UPDATE public.recipe_items ri
       SET unit_cost = ROUND(COALESCE(inv.cost_per_recipe_unit, 0)
                             * public.unit_convert_factor(ri.unit, COALESCE(inv.recipe_unit, ri.unit)), 8),
           subtotal = ROUND(COALESCE(ri.quantity, 0) * COALESCE(inv.cost_per_recipe_unit, 0)
                            * public.unit_convert_factor(ri.unit, COALESCE(inv.recipe_unit, ri.unit)), 6),
           updated_at = now()
      FROM public.recipes r
     ...
     WHERE r.id = ANY(v_touched)
       ...
       AND COALESCE(public.unit_convert_factor(ri.unit, COALESCE(inv.recipe_unit, ri.unit)), 0) <> 0;
