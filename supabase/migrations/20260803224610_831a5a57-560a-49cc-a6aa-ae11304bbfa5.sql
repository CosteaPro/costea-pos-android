...
AS $function$
DECLARE
  v_nv bigint; v_prod bigint; v_sup bigint; v_inv bigint; v_fact integer;
BEGIN
  ...
  UPDATE public.company_settings
     SET next_sequential = GREATEST(COALESCE(next_sequential, 1), v_fact + 1), updated_at = now();

  RETURN jsonb_build_object('nota_venta', v_nv, 'producto', v_prod, 'proveedor', v_sup,
    'inventario', v_inv, 'factura', v_fact);
END; $function$;

REVOKE ALL ON FUNCTION public.resync_sequences() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.resync_sequences() TO service_role;