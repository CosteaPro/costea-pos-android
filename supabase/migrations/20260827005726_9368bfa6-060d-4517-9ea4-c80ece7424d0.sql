...
BEGIN
  ...
  -- Lineas de item: costo convertido a la unidad exacta de la linea.
  UPDATE public.recipe_items ri
     SET unit_cost = ROUND(COALESCE(i.cost_per_recipe_unit, 0) * f.factor, 8),
         subtotal = ROUND(COALESCE(ri.quantity, 0) * COALESCE(i.cost_per_recipe_unit, 0) * f.factor, 6),
         updated_at = now()
    FROM public.inventory_items i
    CROSS JOIN LATERAL (
      SELECT public.unit_convert_factor(COALESCE(i.recipe_unit, ri.unit), ri.unit) AS factor
    ) f
   WHERE i.id = _item_id
  ...
  WHILE v_level < 6 AND COALESCE(array_length(v_touched, 1), 0) > 0 LOOP
    ...
    -- Lineas que consumen esas recetas como subreceta/receta (costo por unidad de rendimiento).
    UPDATE public.recipe_items ri
       SET unit_cost = ROUND(c.costo * f.factor, 8),
           subtotal = ROUND(COALESCE(ri.quantity, 0) * c.costo * f.factor, 6),
           updated_at = now()
      FROM (
      ...
      CROSS JOIN LATERAL (
        SELECT public.unit_convert_factor(COALESCE(c.yield_unit, ri.unit), ri.unit) AS factor
      ) f
    ...
    UPDATE public.recipe_items ri
       SET unit_cost = ROUND(COALESCE(inv.cost_per_recipe_unit, 0) * f.factor, 8),
           subtotal = ROUND(COALESCE(ri.quantity, 0) * COALESCE(inv.cost_per_recipe_unit, 0) * f.factor, 6),
           updated_at = now()
      FROM public.recipes r
      JOIN public.inventory_items inv ON inv.id = r.inventory_item_id
      CROSS JOIN LATERAL (
        SELECT public.unit_convert_factor(COALESCE(inv.recipe_unit, ri.unit), ri.unit) AS factor
      ) f
     WHERE r.id = ANY(v_touched)
       AND ri.source_type = 'item'
