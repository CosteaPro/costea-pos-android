# La fecha de compras sale como 30 cuando estamos 29

Exacto: en Ecuador son las 21:27 del **29 de agosto**, pero el formulario propone 30/08.

## Por qué pasa

El formulario toma la fecha del reloj **universal (UTC)**, que va 5 horas adelante. Desde las
19:00 de Ecuador, UTC ya está en el día siguiente, así que propone 30/08 y la compra se guarda
con fecha de mañana.

Eso es lo que causó la diferencia entre el P&G y el inventario:
...
2. **Guardar la compra con la fecha elegida en hora de Ecuador**, para que caiga siempre en el
   día contable correcto sin depender del reloj del equipo.
3. **Los filtros de fecha del historial de compras** también usan el día contable de Ecuador.
4. **Corregir las tres compras del 30/08** ya registradas, moviéndolas al 29/08. Tras eso, P&G
   e inventario coinciden solos.
5. El "Hoy: …" de la pantalla de nueva compra pasa a mostrar el día de Ecuador.
...
- `src/components/admin/purchasing.tsx`: reemplazar `new Date().toISOString().slice(0, 10)`
  ...
  y en el rótulo "Hoy". Guardar `purchased_at` como `` `${emissionDate}T12:00:00-05:00` `` en
  vez de construir la fecha con la zona del navegador. Los filtros `from`/`to` pasan a usar
